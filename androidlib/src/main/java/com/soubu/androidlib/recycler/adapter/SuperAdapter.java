...
public class SuperAdapter extends RecyclerView.Adapter<BaseViewHolder> {
        ...
        holder.getRootView().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (dataDisplayer != null) {
                    dataDisplayer.onItemClick(holder, wrapper.getData(), position);
                }
            }
        });
        return holder;
    ...
    public void onBindViewHolder(BaseViewHolder holder, int position) {
        ...
        DataDisplayer dataDisplayer = wrapper.getHolder();
        if (dataDisplayer != null) {
            dataDisplayer.bindData(holder, wrapper.getData(), position);

            boolean hasListener = listenerMap.get(position, false);
            if (!hasListener) {
                dataDisplayer.bindListener(holder, wrapper.getData(), position);
